			...
			async.series([
				...
				if (!conf.bLight && !isCorrectHash(objUnit, unit)){
					...
					if(objUnit.arrShareDefinition && objUnit.arrShareDefinition.length>0){
						delete objUnit.arrShareDefinition;
						if (!conf.bLight && !isCorrectHash(objUnit, unit)){
							return retryOrError();
						}
					}
...
function readTimestampOfLastMci(conn, mci, handleTimestamp){
	conn.query("SELECT int_value AS timestamp FROM data_feeds JOIN units USING(unit) \n\
	WHERE main_chain_index=? AND feed_name='timestamp' AND pow_type=? AND is_on_main_chain=1 \n\
	AND sequence='good' AND is_stable=1 ", 
		[mci, constants.POW_TYPE_TRUSTME],
		function(rows){
